<script>
  ...
  import infoSessionsData from '$lib/data/info_session_lab.js';
  ...
  // Function to parse German date format DD.MM.YYYY to Date object
  function parseGermanDate(dateStr) {
    const [day, month, year] = dateStr.split('.');
    return new Date(year, month - 1, day);
  }

  // Find the next upcoming info session
  let nextInfoSession = $derived.by(() => {
    const now = new Date();
    now.setHours(0, 0, 0, 0);

    const upcomingSessions = infoSessionsData
      .map((session) => ({
        ...session,
        dateObj: parseGermanDate(session.date),
      }))
      .filter((session) => session.dateObj >= now)
...
>
  ...
  {#if $locale === 'de' && nextInfoSession}
    <BannerAnnouncement top="{$headerHeight > 63 ? 0 : $headerHeight}px;">
      <a href="https://lab.correlaid.org">{@html LabLogo}</a>
      <div class="flex flex-col gap-1">
        <p>
          <strong>
            Registriere deine NPO für die zweite Runde des CorrelLab!
          </strong>
        </p>
        <p>
          Nächste Infoveranstaltung am {nextInfoSession.date}.
          <a
            href="https://pretix.eu/correlaid/correllab-info/"
            style="text-decoration: underline;">Jetzt anmelden</a
          >
          oder
          <a
